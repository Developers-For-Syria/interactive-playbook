...
function Index() {
  ...
  return (
    <div dir="rtl" lang="ar" style={{ fontFamily: "'Cairo', sans-serif", backgroundColor: C.surface }} className="min-h-screen antialiased flex flex-col items-center">
      {/* Progress */}
      <div className="fixed top-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] z-[60] h-1.5" style={{ backgroundColor: C.cardBorder }}>
      ...
      <main className="w-full max-w-[480px] mt-16 px-5 pt-12 pb-32 flex-grow">
        <AnimatePresence mode="wait" custom={direction}>
          <motion.div key={step} custom={direction} initial={{ opacity: 0, x: direction * 60 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: direction * -60 }} transition={{ duration: 0.25, ease: 'easeInOut' }}>
            {step === 0 && <GenderStep data={data} onChange={onChange} />}
            {step === 1 && <BasicInfoStep data={data} onChange={onChange} />}
            {step === 2 && <FocusAreaStep data={data} onChange={onChange} />}
            {step === 3 && <GoalStep data={data} onChange={onChange} />}
            {step === 4 && <FitnessLevelStep data={data} onChange={onChange} />}
            {step === 5 && <ScheduleStep data={data} onChange={onChange} />}
            {step === 6 && <DietStep data={data} onChange={onChange} />}
            {step === 7 && <HealthStep data={data} onChange={onChange} />}
            {step === 8 && <LifestyleStep data={data} onChange={onChange} />}
            {step === 9 && <NotesStep data={data} onChange={onChange} />}
            {step === 10 && <SuccessStep />}
          </motion.div>
        </AnimatePresence>
      </main>
      ...
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] z-50 px-5 pb-8 pt-4 bg-white/80 backdrop-blur-md rounded-t-3xl border-t border-slate-100 shadow-[0_-8px_30px_rgb(26,86,255,0.12)]">
        <button onClick={isSuccess ? () => alert('تم إرسال البيانات بنجاح! 🎉') : next} className="w-full rounded-full py-4 flex justify-center items-center gap-2 font-bold text-lg text-white hover:brightness-110 transition-all shadow-lg shadow-blue-200 active:scale-[0.98]" style={{ backgroundColor: C.primaryContainer, fontFamily: "'Lexend', sans-serif" }}>
          <span>{isSuccess ? 'إرسال البيانات' : 'التالي'}</span>
        ...
        {(['male', 'female'] as const).map((g) => {
          ...
          return (
            <button key={g} onClick={() => onChange({ gender: g })} className={`flex flex-col items-center p-6 rounded-xl border-2 transition-all active:scale-95 relative overflow-hidden`} style={{ backgroundColor: selected ? C.primaryLight : '#fff', borderColor: selected ? C.primaryContainer : C.cardBorder }}>
              {selected && <div className="absolute top-3 left-3 rounded-full w-6 h-6 flex items-center justify-center text-white" style={{ backgroundColor: C.primaryContainer }}><span className="material-symbols-outlined text-[16px]" style={{ fontVariationSettings: "'FILL' 1" }}>check</span></div>}
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="relative">
            ...
            <div className="absolute inset-y-0 left-4 flex items-center" style={{ color: C.textMuted }}><span className="material-symbols-outlined">person</span></div>
          </div>
        </div>
    ...
    <div className="w-full">
      <section className="mb-8 text-center">
        <div className="relative w-24 h-24 mx-auto mb-6">
          ...
          <div className="relative w-full h-full bg-white rounded-3xl shadow-lg flex items-center justify-center border" style={{ borderColor: C.primaryLight }}>
            <span className="material-symbols-outlined text-4xl" style={{ color: C.primaryContainer, fontVariationSettings: "'FILL' 1" }}>restaurant</span>
          </div>
        </div>
    ...
    <div className="w-full">
      ...
      <section className="my-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[{ id: 'office', emoji: '💼', label: 'مكتبي' }, { id: 'active', emoji: '🚶', label: 'نشاط وحركة' }].map((w) => {
            ...
            return (
              <button key={w.id} onClick={() => onChange({ workNature: w.id })} className="rounded-2xl p-6 text-center flex flex-col items-center gap-2 border-2 transition-colors" style={{ backgroundColor: sel ? C.primaryLight : '#fff', borderColor: sel ? C.primaryContainer : C.cardBorder }}>
      ...
      <section className="mb-8">
        ...
        <div className="bg-white rounded-2xl p-6 border shadow-sm" style={{ borderColor: C.cardBorder }}>
          ...
          <div className="flex justify-between w-full text-sm font-semibold" style={{ color: C.textMuted }}><span>مرتفع</span><span>منخفض</span></div>
        </div>
      </section>
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-6 border shadow-sm flex items-center justify-between" style={{ borderColor: C.cardBorder }}>
        <button onClick={() => onChange(Math.min(max, value + 1))} className="w-12 h-12 rounded-full border-2 flex items-center justify-center active:scale-90 transition-transform" style={{ borderColor: C.primaryContainer, color: C.primaryContainer }}><span className="material-symbols-outlined">add</span></button>
        ...
        <button onClick={() => onChange(Math.max(min, value - 1))} className="w-12 h-12 rounded-full border-2 flex items-center justify-center active:scale-90 transition-transform" style={{ borderColor: C.disabled, color: C.disabled }}><span className="material-symbols-outlined">remove</span></button>
      </div>
    </div>
  );
}
